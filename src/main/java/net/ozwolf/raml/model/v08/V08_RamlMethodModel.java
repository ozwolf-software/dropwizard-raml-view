...
public class V08_RamlMethodModel implements RamlMethodModel {
    ...
    public List<RamlParameterModel> getParameters() {
        ...
        if (method.resource().uriParameters() != null)
            method.resource().uriParameters().stream().forEach(p -> parameters.add(new V08_RamlParameterModel.Path(p)));

        if (method.resource().baseUriParameters() != null)
            method.resource().baseUriParameters().stream().forEach(p -> parameters.add(new V08_RamlParameterModel.Path(p)));

        if (method.queryParameters() != null)
            method.queryParameters().stream().forEach(p -> parameters.add(new V08_RamlParameterModel.Query(p)));

        if (method.headers() != null)
            method.headers().stream().forEach(p -> parameters.add(new V08_RamlParameterModel.Header(p)));
    ...
    public List<RamlRequestModel> getRequests() {
        List<RamlParameterModel> headers = method.headers() != null ? method.headers().stream().map(V08_RamlParameterModel.Header::new).collect(toList()) : newArrayList();

        return method.body() != null ?
                method.body().stream().map(b -> new V08_RamlRequestModel(b, headers)).collect(toList()) :
                newArrayList();
    }
        ...
        return method.responses() != null ?
                method.responses()
                        ...
                        .flatMap(r -> {
                            ...
                            List<RamlParameterModel> responseHeaders = r.headers() != null ? r.headers().stream().map(V08_RamlParameterModel.Header::new).collect(toList()) : newArrayList();
                            return r.body().stream().map(b -> new V08_RamlResponseModel(b, code, description, responseHeaders));
                        })
                        .collect(toList()) :
                newArrayList();
    }
